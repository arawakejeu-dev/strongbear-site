...
export default async function KidsPage() {
  ...
  return <>
    ...
    <main className="kids-page" id="contenu">
      ...
      <section className="kids-parent-section" id="parents"><Container className="kids-parent-grid"><div><p className="eyebrow eyebrow-inverse">Pour les parents</p><h2>Vous confiez plus qu’un emploi du temps.</h2><p>Vous devez savoir où votre enfant va, comment il est accompagné et ce que l’on attend de lui. Notre rôle est de rendre ce cadre clair dès le premier échange.</p><ButtonLink href="/academy/enfants-parents" variant="ghost">Consulter les guides parents</ButtonLink></div><div className="kids-parent-points">{parentBenefits.map((benefit) => <article key={benefit.title}><Icon icon={benefit.icon} /><div><h3>{benefit.title}</h3><p>{benefit.copy}</p></div></article>)}</div></Container></section>
