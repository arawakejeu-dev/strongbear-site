...
const FIGHTY_URL = practicalInfo.fightyUrl;
...
const navigationItems = [
  { label: "Jiu-Jitsu", href: "/jiu-jitsu-bresilien-marines" },
  { label: "Grappling", href: "/grappling-marines" },
  { label: "MMA", href: "/mma-marines" },
...
export function Header({ variant = "default" }: { variant?: "default" | "kids" }) {
  ...
  return <header className={`site-header ${isKids ? "site-header-kids" : ""}`}><a className="skip-link" href="#contenu">Aller au contenu</a><Container className="header-inner">
    ...
    <div className="header-actions"><FightyCTA label="Cours d’essai" /><details className="mobile-menu"><summary aria-label="Ouvrir la navigation"><Icon icon={Menu} /><span className="sr-only">Menu</span></summary><div className="mobile-panel"><Navigation items={items} /><FightyCTA label="Réserver le cours d’essai" /></div></details></div>
  </Container></header>;
}
...
export function FightyJourney({ title = "Votre premier cours. Simplement.", ctaLabel = "Réserver mon cours d’essai", steps = fightySteps }: { title?: string; ctaLabel?: string; steps?: Array<{ icon: LucideIcon; title: string; copy: string }> }) {
...
export function Footer({ variant = "default" }: { variant?: "default" | "kids" }) {
  const isKids = variant === "kids";
  return <footer className={`footer ${isKids ? "footer-kids" : ""}`} id="contact"><Container><div className="footer-top"><div><a className="brand" href="/"><strong>STRONGBEAR</strong><span>{isKids ? "KIDS · JIU-JITSU · GRAPPLING" : "BJJ · GRAPPLING · MMA"}</span></a><p>{isKids ? "Un espace sûr pour apprendre, grandir et prendre confiance." : "L’académie d’arts martiaux du Vexin."}</p><span className="footer-location"><Icon icon={MapPin} size="sm" />Marines · Val-d’Oise</span><a href={`mailto:${practicalInfo.contact.email}`}>{practicalInfo.contact.email}</a></div><div className="footer-cta"><span>{isKids ? "Prêt pour une première découverte ?" : "Prêt à progresser ?"}</span><FightyCTA label={isKids ? "Réserver l’essai de mon enfant" : "Réserver mon essai"} /></div></div><div className="footer-grid">{isKids ? <><nav aria-label="Programme Kids"><strong>Programme Kids</strong><a href="#benefices">Bénéfices</a><a href="#pedagogie">Pédagogie</a><a href="#ages">Groupes d’âge</a></nav><nav aria-label="Informations parents"><strong>Parents</strong><a href="#planning">Planning</a><a href="#tarifs">Tarifs</a><a href="#faq">Questions fréquentes</a></nav></> : <><nav aria-label="Disciplines"><strong>Disciplines</strong><a href="/jiu-jitsu-bresilien-marines">Jiu-Jitsu Brésilien</a><a href="/grappling-marines">Grappling</a><a href="/mma-marines">MMA</a></nav><nav aria-label="Découvrir"><strong>Découvrir</strong><a href="/kids">Kids</a><a href="/academy">Academy</a><a href="/#strongbear">À propos</a><a href="/#contact">Contact</a></nav></>}<nav aria-label="Informations"><strong>Informations</strong><a href="/academy/bien-debuter">Bien débuter</a><a href={FIGHTY_URL} rel="external" data-conversion="fighty" data-cta-label="Footer · Fighty">Fighty</a><a href={practicalInfo.contact.instagram} rel="external">Instagram<span className="sr-only"> — compte Strongbear BJJ</span></a>{GOOGLE_BUSINESS_URL && <a href={GOOGLE_BUSINESS_URL} rel="external">Google Business</a>}{PRIVACY_URL && <a href={PRIVACY_URL}>Confidentialité</a>}{LEGAL_URL && <a href={LEGAL_URL}>Mentions légales</a>}</nav></div><div className="footer-bottom"><span>© 2026 Strongbear BJJ & Grappling</span><span>{isKids ? "Confiance · Respect · Plaisir" : "Technique · Respect · Progression"}</span></div></Container></footer>;
}
